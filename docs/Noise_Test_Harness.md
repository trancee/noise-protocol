...
- Swift Testing and the repository iOS wrapper smoke test now also round-trip a representative `Noise_NN_448_ChaChaPoly_SHA256` official-format document through the iOS converter.
...
- Representative official-format parity coverage currently includes `Noise_NN_25519_ChaChaPoly_SHA256`, `Noise_NN_448_ChaChaPoly_SHA256`, `Noise_NNpsk0_25519_ChaChaPoly_SHA256`, and `Noise_XXpsk2_25519_ChaChaPoly_SHA256` through the sample inputs under `scripts/testdata/`.
- `scripts/verify-cross-platform-interop.sh` now also runs representative deterministic shared-fixture execution for `Noise_NN_448_ChaChaPoly_SHA256` on both Android and iOS before the official conversion parity step.
