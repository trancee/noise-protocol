...
echo "[official-convert-ios] Converting $input_path into shared fixtures under $output_directory"
(
  cd "$repo_root/ios"
  swift run NoiseVectorConverterCLI \
    --input "$input_path" \
    --output-dir "$output_directory" \
    --schema-path "$schema_path"
)

echo "[official-convert-ios] Conversion complete."